...
const FILE = `CRIwqt4+szDbqkNY+I0qbDe3LQz0wiw0SuxBQtAM5TDdMbjCMD/venUDW9BL
...
func main() {
	enc, err := base64.StdEncoding.DecodeString(FILE)
	if err != nil {
		log.Fatal(err)
	}
	...
	bs := c.BlockSize()
	dst := make([]byte, bs)
	for i := 0; i < len(enc)/bs; i++ {
		c.Decrypt(dst, enc[i*bs:])
		os.Stdout.Write(dst)
	}
	os.Stdout.Write([]byte{'\n'})
	fmt.Println("rest bytes:", len(enc)%bs)
}
